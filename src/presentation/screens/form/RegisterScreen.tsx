...
export const RegisterScreen = () => {
  ...
  const saveRegister = async () => {
    ...
    try {
      ...
      if (json.result) {
        Alert.alert('MINSA', 'Registrado con éxito.');
        navigation.navigate('HomeScreen');  
      } else {
        Alert.alert('MINSA', 'No se realizó el registro. Contacte a su Administrador.');
      }
    } catch (error) {
    ...
    const requestLocationPermission = async () => {
      ...
      try {
        ...
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
          ...
          Alert.alert('MINSA', 'Permiso de ubicación denegado');
        }
      } catch (error) {
        Alert.alert('MINSA', `${error}`);
        setGeoPosting(false);
  ...
  if (data.length === 0 || espacio.length === 0 || implementacion.length === 0 ) {
  ...
  return (
    <Layout style={{ flex: 1 }}>
      <TopNavigation
        title={props => <Text style={[{ color: 'white', fontWeight: 'bold' }]}>REGISTRAR EPSJ</Text>}
        alignment="center"
      ...
      <ScrollView 
        ...
        <Layout style={{ marginTop: 30, marginBottom: 30 }}>
          <Button
            disabled={isPosting}
            accessoryRight={<MyIcon name="save-outline" color={isPosting ? '#484E59' : '#FFFFFF'} />}
            style={{ backgroundColor: '#191919', borderColor: '#191919' }}
            onPress={saveRegister}>FINALIZAR REGISTRO</Button>
        </Layout>
      </ScrollView>
    </Layout>
  );
